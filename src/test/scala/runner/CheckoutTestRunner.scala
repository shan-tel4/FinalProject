...
@CucumberOptions( // Begin Cucumber configuration block
  ...
  plugin = Array("pretty", //  • “pretty” prints colour‑coded steps to the console
    "html:target/Checkout-Tests/Checkout Test Report.html" //  • Generates an HTML report at target/cucumber-report.html
  ),
...
class CheckoutTestRunner // Empty body is fine; class merely acts as a configuration holder
