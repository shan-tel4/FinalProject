...
@CucumberOptions( // Begin Cucumber configuration block
  ...
  plugin = Array("pretty", //  • “pretty” prints colour‑coded steps to the console
    "html:target/Footer-Tests/Footer Test Report.html" //  • Generates an HTML report at target/cucumber-report.html
  ),
...
class FooterTestRunner // Empty body is fine; class merely acts as a configuration holder
