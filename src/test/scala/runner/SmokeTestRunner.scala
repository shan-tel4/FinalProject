...
@CucumberOptions( // Begin Cucumber configuration block
  ...
  plugin = Array("pretty", //  • “pretty” prints colour‑coded steps to the console
    "html:target/Smoke-Tests/Smoke Test Report.html" //  • Generates an HTML report at target/cucumber-report.html
  ),
...
class SmokeTestRunner // Empty body is fine; class merely acts as a configuration holder
